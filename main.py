...
dala = commands.Bot(command_prefix=commands.when_mentioned_or('dala ', 'dala'), help_command = None)
...
@dala.command()
async def ping(ctx):
    await ctx.send(f'Pong! `{round(dala.latency * 1000)}`ms')
...
async def on_message(message):
    if message.author.bot == False:
        ...
        with open('users.json', 'w') as f:
            json.dump(users, f)
            await dala.process_commands(message)
...
strings = {'dala', 'dancingdala', 'dancing dala'}

@dala.event
async def on_message(message):
  replytomention = ('who called upon me?', 'dont call me, im already having very low space in heroku TwT',
   'im sorey im not that gud of a bot Ow0', 'me having shutdown thoughts', 'heroku wont kill me by going offline right?')
  if dala.user.mentioned_in(message):
    await message.channel.send(random.choice(replytomention))
...
async def spam(ctx):
  ...
  await ctx.send('     ')
  await ctx.send('     ')
  await ctx.send('https://gfycat.com/sphericallighthornedtoad')
  embed = discord.Embed(color=discord.Color.purple())
  embed.set_footer(icon_url=ctx.author.avatar_url, text=f'idha spam panni iriitate panna moodhevi, {ctx.author.name}')
